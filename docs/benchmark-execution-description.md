...
Команда не поднимает Docker локально, не перезапускает сервер и не меняет живой `agent_config`, модель, базу или каталог навыков. По SSH она **append-only** пинит два бенчмарк-конфига (`general_knowledge` без инструментов и `existing_skills` с шестью) от текущего `agent_config`, проверяет роли через `/control/scope` и проверяет навыки **на сервере** теми же валидаторами, что используются локально. Возвращаются только настройки, хеши и результаты проверки; полный каталог и `truth/` не скачиваются. `generated_skills` по-прежнему mock и в remote не запускается.

После preflight клиент для каждого режима создаёт новую сессию по HTTPS с соответствующим pinned `config_ref` и передаёт `query` с публичным контрактом, без gold. Исходные JSON не изменяются. Несовпадение snapshot, реестра или роли останавливает запуск до LLM. Файлы результатов такие же, как у локального benchmark, в `benchmarking/results/remote-.../`.
...
- `benchmark-remote` требует SSH и HTTPS к серверу, не использует Compose, пинит два бенчмарк-конфига и запускает `general_knowledge` и `existing_skills`. `generated_skills` остаётся mock.
