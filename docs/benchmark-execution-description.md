...
Smoke проверяет полный технический путь `case → agent → information service → trace → normalization → metrics`. Низкая accuracy не считается технической ошибкой. Ошибка стенда, несовпадение fingerprint или ненормализуемый ответ приводят к ненулевому коду завершения. Успешный отказ харнесса (`denied:Bash` и аналоги) при готовом ответе не считается ошибкой прогона: граница инструментов сработала, ход завершился.
...
Доступен только `existing_skills`: текущая серверная конфигурация должна разрешать стандартные шесть инструментов, запрещать выполнение кода и использовать stateless-сессии. `skills_disabled` потребовал бы отдельной конфигурации на сервере; команда её не создаёт. Сравнения режимов в таком прогоне нет.
...
Трасса читается напрямую через `/phoenix/v1/projects/b2e-sim/spans`, минуя ошибочную фильтрацию старого research-api. Используются `trace_id` ответа, проверка `session.id`, пагинация и отбрасывание чужих spans. Клиент ждёт завершённый root и два одинаковых чтения; это снижает риск задержки экспорта, но не заменяет гарантию доставки всех spans самим Phoenix. Недоступная или превышающая лимит трасса считается ошибкой прогона, а не нулём инструментальных вызовов.
...
- `BENCH_RESULTS`, `BENCH_TIMEOUT`, `BENCH_EVAL_ID`, `BENCH_REPETITIONS` — как у остальных запусков.
