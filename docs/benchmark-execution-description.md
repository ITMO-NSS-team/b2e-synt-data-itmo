...
Трасса читается напрямую через `/phoenix/v1/projects/b2e-sim/spans`, минуя ошибочную фильтрацию старого research-api. Используются `trace_id` ответа, проверка `session.id`, пагинация и отбрасывание чужих spans. Клиент ждёт, пока трасса станет полной: свой `session.id`, завершённый root, число Heimdall-spans не меньше `heimdall_calls` агента и два одинаковых чтения подряд. Пока снимок растёт, опрос продолжается. `--trace-attempts` — это число одинаковых неполных снимков подряд, после которых клиент сдаётся, а не фиксированное окно в секундах. Недоступная трасса не обнуляет инструментальные вызовы: ячейка получает статус `unscored` и уходит в `review`.
